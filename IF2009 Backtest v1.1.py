                ...
                if variable0 == 0:  # 0是初始值
                    ...
                    if signal.at[index, 'withdrawal_signal']:
                        if signal.at[index, 'increase_signal']:
                            ...
                            for i in reversed(range(last_index, integer_index + 1)):  # 循环会在integer_index处停止
                            ...
                            # l_price = min(underlying[last_index: integer_index + 1]['low'])
                            # low_index = underlying[last_index: integer_index + 1][underlying[last_index: integer_index + 1]['low'] == l_price].index
                            # print(low_index)
                            # print(l_index)
                ...
                if variable0 == 3:
                    ...
                    for i in range(start_index + 1, integer_index + 2):  # last_index+1是为了在第一次计算时值为last_index所在位置的值
                        ...
                        if increase4 == increase3:
                            high_index = i - 1
                            break
                    # testing = underlying[start_index+1: integer_index + 2]['high'].idxmax()
                    # print(high_index)
                    # print(testing)
...
for num in range(for_num_1):
    ...
    # Parameters
        ...
        print('\n')
        # strategy parameters
        open_minute = close_minute = 100 + (i*step1)
        open_threshold = close_threshold = 5
        open_continous_threshold = 10
        open_withdrawal_threshold = close_withdrawal_threshold = 5 + (num * step2)
